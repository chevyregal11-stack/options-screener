...
function polygonGet(path) {
  return new Promise((resolve, reject) => {
    ...
    const options = { hostname: BASE, path: fullPath, method: "GET" };
...
exports.handler = async (event) => {
  ...
  if (!TOKEN) {
    return {
      statusCode: 500,
      headers,
      body: JSON.stringify({ error: "POLYGON_API_KEY not set in environment variables" }),
    };
  }
  ...
  const {
    tickers = ["SPY", "QQQ", "NVDA", "TSLA", "AAPL", "SPX"],
    filters = {},
  } = body;
  ...
  for (const ticker of tickers) {
    try {
      const underlying = ticker === "SPX" ? "I:SPX" : ticker;
      ...
      while (hasMore) {
        ...
        if (data.status === "ERROR" || !data.results) {
          console.error(`Polygon error for ${ticker}:`, data.error || data.message || "");
          break;
        }
